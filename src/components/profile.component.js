...
export default class Profile extends Component {
  ...
  preCal() {
    ...
    if (diffDays > 365)
      if (Math.floor(diffDays / 365) === 1) res = "a year ago";
      else res = Math.floor(diffDays / 365) + " years ago";
    else if (diffDays > 30)
      if (Math.floor(diffDays / 30) === 1) res = "a month ago";
      else res = Math.floor(diffDays / 30) + " months ago";
    else if (diffDays != 0)
      if (diffDays === 1) res = "a day ago";
      else res = diffDays + " days ago";
    else res = "today";
    // console.log(res);

    const { rating } = this.state;
    var color = "black";
    if (rating < 100) color = "gray";
    else if (rating < 200) color = "blue";
    else if (rating < 300) color = "purple";
    else if (rating < 400) color = "orange";
    else color = "red";
    this.setState({
      register: res,
      color: color,
  ...
  render() {
      ...
      return (
        <div className="Profile">
          <div className="NavigateButton">
            <Button onClick={this.toProfilePage}>PROFILE</Button>
            <Button onClick={this.toSettingPage}>SETTING</Button>
          </div>
          {this.state.page == 0 && (
    ...
    else return <p> Please log in to View your profile</p>;
  }
}
