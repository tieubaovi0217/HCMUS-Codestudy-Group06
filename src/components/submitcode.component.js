...
export default class SubmitCode extends Component {
  ...
  submitOneTest = async (submissionInfo, index) => {
    const response = await fetch(`${this.MAIN_HOST1}${this.MAIN_HOST2}`, {
      method: "POST",
      headers: {
  ...
  submit = async (e) => {
    ...
    if (this.state.inputs == null) {
      ...
      await axios
        .post("http://localhost:5000/viewsubmission/last-submission-id")
        .then((response) => {
          // console.log((parseInt(response.data[0].submission_id) + 1).toString().padStart(5, '0'));
          ourSubmission.submission_id = (
            parseInt(response.data[0].submission_id) + 1
          )
            .toString()
            .padStart(5, "0");
        })
      ...
      await axios
        .post("http://localhost:5000/viewsubmission/add", ourSubmission)
        .then((response) => {
      ...
      for (const result of ourResults) {
        ...
        if (parseFloat(result.time) > finalTime)
          finalTime = parseFloat(result.time);
      ...
      axios
        .post("http://localhost:5000/viewsubmission/modify", newSubmission)
        .then((response) => {
          console.log(response);
        })
        .catch((error) => {
  ...
  render() {
    if (this.isLoggedIn) {
      ...
      return <p> Please log in to Submit Code</p>;
    }
  }
}
